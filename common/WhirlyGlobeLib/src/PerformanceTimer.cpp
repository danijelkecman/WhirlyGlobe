...
void PerformanceTimer::log(double min)
{
  ...
  char line[1024]; // Ensure this buffer is large enough for your needs
  
  for (const auto &entry : sortedEntries) {
    if (entry.numRuns > 0 && entry.maxDur >= min) {
      snprintf(line, sizeof(line), "%s: min, max, mean = (%.3f, %.3f, %.4f) ms, %d reports",
               entry.name.c_str(), 1000 * entry.minDur, 1000 * entry.maxDur, 1000 * entry.avgDur / entry.numRuns, entry.numRuns);
      report(line);
    }
  }
  ...
  for (const auto &countEntry : countEntries) {
    ...
    if (entry.numRuns > 0 && entry.maxCount > 0) {
      snprintf(line, sizeof(line), "%s: min, max, mean (%d, %d, %.3f), %d reports",
               entry.name.c_str(), entry.minCount, entry.maxCount, (float)entry.avgCount / (float)entry.numRuns, entry.numRuns);
      report(line);
    }
  }
}
